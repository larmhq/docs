...
<ParamField body="monitors" type="object[]">
  Monitors to link. Each object has `id` (monitor UUID) and `down_status` (`major_outage`, `partial_outage`, or `degraded_performance`). Linked monitors trigger auto-disruptions when they detect failures.
</ParamField>
